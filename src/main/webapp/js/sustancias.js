...
.controller ('SustanciasCtrl',['$scope','$http','ngTableParams','$filter', function ($scope,$http,ngTableParams,$filter){
	
	$scope.listas = {};
	$scope.formulario ={};
	$scope.tablas ={};
	$scope.tablas.combinacion = [];
	$scope.insertar={};
	$scope.update={};
	$http({
		method: "POST",
		url:"http://localhost:8080/Practica_WebApp/services/clinica/combinacion"
	}).success(function(data){
		 $scope.tablas.combinacion = data;
	...
	$scope.tableParams = new ngTableParams({
	    ...
	    total: $scope.tablas.combinacion.length, // length of data
		getData: function($defer, params) {
			
			var filteredData = params.filter() ? $filter('filter')($scope.tablas.combinacion, params.filter().myfilter) : data;
	...
	$scope.$watch("tablas.combinacion", function(){
	      $scope.tableParams.reload(); 
	      console.log($scope.tablas.combinacion);
	  })
